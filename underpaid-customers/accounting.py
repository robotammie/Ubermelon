...
def check_accounting(file_name):
    """checks the list of customer orders and prints out all over- and underpayments"""

    melon_cost = 1.00

    order_report = open('customer-orders.txt')

    for line in order_report:
        line = line.rstrip()
        words = line.split('|')
        ...
        if expected != paid:
            print name, "paid {:.2f}, expected {:.2f}".format(
                paid, expected)


check_accounting('customer-orders.txt')
